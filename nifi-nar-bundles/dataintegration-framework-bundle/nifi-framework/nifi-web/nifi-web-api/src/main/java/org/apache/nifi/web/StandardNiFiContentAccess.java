...
public class StandardNiFiContentAccess implements ContentAccess {
    ...
    public DownloadableContent getContent(final ContentRequestContext request) {
        ...
        if (properties.isClustered() && clusterCoordinator != null && clusterCoordinator.isConnected()) {
            ...
            // example URIs:
            // http://localhost:60017/dataintegration-api/provenance/events/{id}/content/{input|output}
            // http://localhost:60017/dataintegration-api/flowfile-queues/{uuid}/flowfiles/{uuid}/content

            // get just the context path for comparison
            final String dataUri = StringUtils.substringAfter(request.getDataUri(), "/dataintegration-api");
