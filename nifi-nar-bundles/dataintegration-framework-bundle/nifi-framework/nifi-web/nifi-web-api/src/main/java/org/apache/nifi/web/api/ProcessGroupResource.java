...
public class ProcessGroupResource extends ApplicationResource {
    ...
    private boolean waitForProcessorStatus(final URI originalUri, final String groupId, final Set<String> processorIds, final ScheduledState desiredState,
        ...
        try {
            groupUri = new URI(originalUri.getScheme(), originalUri.getUserInfo(), originalUri.getHost(),
                originalUri.getPort(), "/dataintegration-api/process-groups/" + groupId + "/processors", "includeDescendantGroups=true", originalUri.getFragment());
    ...
    private boolean waitForControllerServiceStatus(final URI originalUri, final String groupId, final Set<String> serviceIds,
        ...
        try {
            groupUri = new URI(originalUri.getScheme(), originalUri.getUserInfo(), originalUri.getHost(),
                originalUri.getPort(), "/dataintegration-api/flow/process-groups/" + groupId + "/controller-services", "includeAncestorGroups=false,includeDescendantGroups=true", originalUri.getFragment());
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
    ...
    private void scheduleProcessors(final String groupId, final URI originalUri, final VariableRegistryUpdateRequest updateRequest,
        ...
        try {
            scheduleGroupUri = new URI(originalUri.getScheme(), originalUri.getUserInfo(), originalUri.getHost(),
                originalUri.getPort(), "/dataintegration-api/flow/process-groups/" + groupId, null, originalUri.getFragment());
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
    ...
    private void activateControllerServices(final String groupId, final URI originalUri, final VariableRegistryUpdateRequest updateRequest,
        ...
        try {
            controllerServicesUri = new URI(originalUri.getScheme(), originalUri.getUserInfo(), originalUri.getHost(),
                originalUri.getPort(), "/dataintegration-api/flow/process-groups/" + groupId + "/controller-services", null, originalUri.getFragment());
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
    ...
    private void applyVariableRegistryUpdate(final String groupId, final URI originalUri, final VariableRegistryUpdateRequest updateRequest,
        ...
        try {
            applyUpdatesUri = new URI(originalUri.getScheme(), originalUri.getUserInfo(), originalUri.getHost(),
                originalUri.getPort(), "/dataintegration-api/process-groups/" + groupId + "/variable-registry", null, originalUri.getFragment());
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
    ...
    public Response createProcessGroup(
        ...
        requestProcessGroupEntity.getComponent().setParentGroupId(groupId);
         if (requestProcessGroupEntity.getComponent().getState() != null) {
        requestProcessGroupEntity.getComponent().setState(requestProcessGroupEntity.getComponent().getState());
          }
        if (isReplicateRequest()) {
    ...
    public Response uploadTemplate(
        ...
        try {
            ...
            processors.forEach((processorDTO) -> {
                decryptProcessorSensitiveValue(processorDTO);
            });
            ...
            controllerServices.forEach((serviceDTO) -> {
                decryptControllerSensitiveValue(serviceDTO);
            });
            ...
            remoteGroupProperty.forEach((remoteProcessGroupDTO) -> {
                decryptRemoteGroupSensitiveValue(remoteProcessGroupDTO);
            });
    ...
    private void updateProcessGroupSensitiveValue(ProcessGroupDTO processGroupDTO){
        ...
        processGroupContents.getControllerServices().forEach((controllerServiceDTO) -> {
            decryptControllerSensitiveValue(controllerServiceDTO);
        });
        processGroupContents.getRemoteProcessGroups().forEach((remoteGroupDTO) -> {
            decryptRemoteGroupSensitiveValue(remoteGroupDTO);
        });
    ...
    private void decryptProcessorSensitiveValue(ProcessorDTO processorDTO){
                    ...
                    processorProperties.entrySet().forEach((entry) -> {
                        ...
                        if (descriptor.getName().equals(processorKey)) {
                            if (processorProperty != null) {
                                processorProperties.put(descriptor.getName(), decryptPassword(processorProperty));
                            }
                        }
                    });
                });
    ...
    private void decryptControllerSensitiveValue(ControllerServiceDTO serviceDTO){
                ...
                controllerProperties.entrySet().forEach((entry) -> {
                    ...
                    if (descriptor.getName().equals(controllerKey)) {
                        if (controllerValue != null) {
                            controllerProperties.put(descriptor.getName(), decryptPassword(controllerValue));
                        }
                    }
                });
            });
    ...
    private void decryptRemoteGroupSensitiveValue(RemoteProcessGroupDTO remoteProcessGroupDTO){
        remoteProcessGroupDTO.setProxyPassword(decryptPassword(remoteProcessGroupDTO.getProxyPassword()));
    }
    
    private String decryptPassword(String password) {
        try {
            StringEncryptor nifiEncryptor = StringEncryptor.createEncryptor(properties);
            return nifiEncryptor.decrypt(password);
        }
        catch(Exception e) {
            return password;
        }
