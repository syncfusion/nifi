...
(function (root, factory) {
    ...
    var nfCanvasUtils = {
        ...
        queryBulletins: function (componentIds) {
            ...
            return $.ajax({
                type: 'GET',
                url: '../dataintegration-api/flow/bulletin-board',
                data: {
                    sourceId: ids
                },
                dataType: 'json'
        ...
        showDeepLink: function (forceCanvasLoad) {
            ...
            if ('URLSearchParams' in window) {
                ...
                if (!nfCommon.isDefinedAndNotNull(urlSearchParams)) {
        ...
        setURLParameters: function (groupId, selections) {
            ...
            if ('URLSearchParams' in window) {
                ...
                // the URL object's `.searchParams` property
                if (!nfCommon.isDefinedAndNotNull(params)) {
...
/**
         * Wrap the string in the specified selection if necessary.
         *
         * @param {selection} selection
         * @param {string} text
         */
       wrapText: function (selection, text) {
            text = text.trim();
            var width = parseInt(selection.attr('width'), 10);
            if (text.length > (width / 5) && text.length > 0) {
                var length = Math.abs((width / 5) - text.length);
        ...
        activeThreadCount: function (selection, d, setOffset) {
            ...
            if (d.status.aggregateSnapshot.activeThreadCount > 0) {
                ...
                if($("#"+id).hasClass("processor")){
                selection.select('text.active-thread-count').style('display', 'block');   
                // update the active thread count
                selection.select('text.active-thread-count')
                    .each(function () {
                         var activeThreadCount = d3.select(this);
                         nfCanvasUtils.ellipsis(activeThreadCount, d.status.aggregateSnapshot.activeThreadCount.toString());
                    })
                    .style('display', 'block');
                // update the background width
                selection.select('text.active-thread-count-icon')
                    .style('display', 'block');
                   // update the active thread text
                selection.select('text.active-thread-status-text')
                    .style('display', 'block');
                }
               else
               {
             // update the active thread count   
              selection.select('text.active-thread-count').style('display', 'block');
               ...
               .style('display', 'block');
                ...
                selection.selectAll('text.active-thread-count, text.active-thread-count-icon,text.active-thread-status-text').style('display', 'none');
            }
        },
        ...
        /**
         * Determines if the graph is currently in a state to publish.
         *
         * @argument {selection} selection    The selection
         */
        canPublish: function (selection) {           
            // if nothing is selected return
            if (selection.empty()) {
                return false;
            }
            ...
            // determine how many publishable components are selected
           var publishable = selection.filter(function (d) {
                var selected = d3.select(this);
                if (nfCanvasUtils.isConnection(selected)) {
                    var sourceIncluded = !selection.filter(function (source) {
                        var sourceComponentId = nfCanvasUtils.getConnectionSourceComponentId(d);
            ...
            // ensure everything selected is publishable
            return selection.size() === publishable.size();
        },
