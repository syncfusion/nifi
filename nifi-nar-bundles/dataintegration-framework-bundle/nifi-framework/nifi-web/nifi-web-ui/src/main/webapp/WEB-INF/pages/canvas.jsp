...
<!DOCTYPE html>
<html style="overflow-y: auto;min-height: 600px;min-width: 800px;">
    <head>
        <title>Syncfusion Data Integration Platform</title>
        ...
        <link rel="stylesheet" href="fonts/Tab/Tabfont.css" type="text/css" />
    ...
    <body ng-controller="ngCanvasAppCtrl" id="canvas-body" style="min-width: 800px;">
        <div id="splash">
        ...
        <jsp:include page="/WEB-INF/partials/delete-confirmation-dialog.jsp"/>
        <jsp:include page="/WEB-INF/partials/publish-template-dialog.jsp"/>
        <jsp:include page="/WEB-INF/partials/add-existing-server-dialog.jsp"/>
        <jsp:include page="/WEB-INF/partials/canvas/change-Password-dialog.jsp"/>
        <jsp:include page="/WEB-INF/partials/canvas/nifi-properties-change-dialog.jsp"/>
        <jsp:include page="/WEB-INF/partials/registry-variables-dialog.jsp"/>
        <div id="canvas-container" class="unselectable" style="min-height: 600px;"></div>
        ...
        <jsp:include page="/WEB-INF/partials/canvas/custom-jar-configuration.jsp"/>
        ...
        <jsp:include page="/WEB-INF/partials/security/add-policy-to-users.jsp"/>
        <jsp:include page="/WEB-INF/partials/security/manage-permission.jsp"/>
        <jsp:include page="/WEB-INF/partials/security/security-management.jsp"/>
        <jsp:include page="/WEB-INF/partials/security/manage-component-policies.jsp"/>
        <jsp:include page="/WEB-INF/partials/security/create-component-policy.jsp"/>
        <jsp:include page="/WEB-INF/partials/security/enable-securityconfiguration-dialog.jsp"/>
        <jsp:include page="/WEB-INF/partials/security/new-user.jsp"/>
        <jsp:include page="/WEB-INF/partials/security/new-usergroup.jsp"/>
        <jsp:include page="/WEB-INF/partials/security/security-delete-dialog.jsp"/>
        ...
        <jsp:include page="/WEB-INF/partials/canvas/data-preview-dialog.jsp"/>
        <div id="context-menu" class="context-menu unselectable"></div>
        <span id="dataintegration-content-viewer-url" class="hidden"></span>
    </body>
</html>
<script>
    (function(){
        if(/*@cc_on!@*/false || !!document.documentMode) //Check if it is IE browser, then show alert message.
            alert("Internet Explorer does not have support to open this URL. Please try with other browser.");
    })();
    
    $(document).click(function(event){
        if(!event.target.matches('.session-common-class'))
        {
            $("#logout-session").css("display","none");
        }
        else{
            var propertyValue = $("#logout-session").css("display");
            if(propertyValue === "block")
                $("#logout-session").css("display", "none");
            else
                $("#logout-session").css("display", "block");
        }
        if(!event.target.matches('.applicationLogo')) {
    ...
    $( document ).ready(function() {                 
            $.ajax({
            type: 'GET',
            url: '../dataintegration-api/syncfusion/reportingTask',
            async: false
        });
    });
</script>