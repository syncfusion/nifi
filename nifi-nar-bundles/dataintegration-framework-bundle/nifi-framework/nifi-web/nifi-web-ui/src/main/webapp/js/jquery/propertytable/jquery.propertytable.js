...
/* global define, module, require, exports, nf, sqlPropertyValue, response */
...
(function (root, factory) {
    ...
    var textEditor = function (args) {
        ...
        this.init = function () {
            ...
            var ok = $('<div class="button">Ok</div>').css({
                // Modified the secondary button background color from #728E9B to #448DD5
                'background': '#448DD5 !important',
                'color': '#FFFFFF !important',
                'border': '1px solid #ccc !important'
            }).hover(
                    function() {
                        $(this).css('background', '#C0D8F0', 'border', '1px solid #448DD5');
                    }, function() {
                $(this).css("background-color", '#448DD5');
            }).on('click', scope.save);
            var cancel = $('<div class="secondary-button">Cancel</div>').css({
                // Modified the secondary button color from #E3E8EB to #fff
               'background': '#FFFFFF',
                'color': '#5C5C5C',
                'border': '1px solid #ccc'
            }).hover(
                    function() {
                        // Modified the secondary button hover color from #C7D2D7,#E3E8EB to #f2f2f2,#fff
            ...
            $('<div></div>').css({
                'position': 'relative',
                // Aligned the property dialog button 
                'top': '-2px',
                'left': '6px',
                'width': '212px',
                'clear': 'both',
    ...
    var nfelEditor = function (args) {
        ...
        this.init = function () {
            ...
            var ok = $('<div class="button">Ok</div>').css({
                ...
                function () {
                    $(this).css('background', '#C0D8F0');
                }, function () {
                    $(this).css('background', '#448DD5');
                }).on('click', scope.save);
            var cancel = $('<div class="secondary-button">Cancel</div>').css({
                'color': 'rgb(92, 92, 92)',
                'background': '#FFFFFF',
    ...
    var comboEditor = function (args) {
        ...
        var cancel = $('<div class="secondary-button">Cancel</div>').css({
           ...
           var ok = $('<div class="button">Ok</div>').css({
            ...
            $('<div></div>').css({
                'position': 'relative',
                //Aligned the secondary button position
                'top': '-2px',
                'left': '6px',
                'width': '212px',
                'clear': 'both',
    ...
    var promptForNewControllerService = function (gridContainer, grid, item, serviceType, bundle, configurationOptions) {
        $.ajax({
            type: 'GET',
            url: '../dataintegration-api/flow/controller-service-types',
            data: {
            ...
            if (options.length === 0) {
                ...
                var create = function () {
                    ...
                    var uri = '../dataintegration-api/controller/controller-services';
                    if (nfCommon.isDefinedAndNotNull(groupId)) {
                        uri = '../dataintegration-api/process-groups/' + encodeURIComponent(groupId) + '/controller-services';
                    }
    ...
    var initPropertiesTable = function (table, options) {
        ...
        var actionFormatter = function (row, cell, value, columnDef, dataContext) {
            ...
            if (options.readOnly !== true && dataContext.type.indexOf('customDefined')>=0){
                 var dataSource = dataContext.type.split("-")[1];
                markup += '<div title="Upload jar" class="download-property  pointer fa fa-upload '+dataSource+'" style="margin-top: 2px" ></div>';
            }
            return markup;
        };
        ...
        var goToControllerService = function (property) {
            $.ajax({
                type: 'GET',
                url: '../dataintegration-api/controller-services/' + encodeURIComponent(property.value),
                dataType: 'json'
        ...
        propertyGrid.onClick.subscribe(function (e, args) {
            if (propertyGrid.getColumns()[args.cell].id === 'value') {
                ...
                if (target.hasClass('delete-property')) {
                    ...
                    // prevents standard edit logic
                    e.stopImmediatePropagation();
                    } else if (target.hasClass('download-property')) {
                        nf.CustomJarConfiguration.jaruploadpopup(target);
        ...
        if (nfCommon.isDefinedAndNotNull(properties)) {
            ...
            $.each(properties, function (name, value) {
                ...
                if (nfCommon.isDefinedAndNotNull(descriptor)) {
                    if (descriptor.name==="MySQL-database-driver-locations"){
                        type = 'customDefined-SQL';
                        SqlPropertyResponse = getSQLPropertyValue("MySQL");
                    } else if(descriptor.name==="PostgreSQL-database-driver-locations"){
                         type = 'required';
                         value= getSQLPropertyValue("PostgreSQL");
                    }else if(descriptor.name==="Oracle-database-driver-locations"){
                       type = 'customDefined-Oracle';
                       SqlPropertyResponse= getSQLPropertyValue("Oracle");
                    }else if(descriptor.name==="MicrosoftSQL-database-driver-locations"){
                       type = 'customDefined-MSSQL';
                       SqlPropertyResponse= getSQLPropertyValue("SQLServer");
                    }else if(descriptor.name==="DBCP-database-driver-locations"){
                       type = 'customDefined-DBCP';
                       SqlPropertyResponse= value;
                    }
                    else if (nfCommon.isRequiredProperty(descriptor)) {
                        type = 'required';
                    ...
                    if(type.indexOf("customDefined")>=0){
                        value=SqlPropertyResponse;
                    }
                }
    ...
    var getSQLPropertyValue = function (databasetype) {
        var sqlPropertyValue = "";
        $.ajax({
            type: 'POST',
            url: '../dataintegration-api/syncfusion/getSQLPropertiesValue',
            dataType: 'text',
            data: {
                dbType: databasetype
            },
           ...
           sqlPropertyValue = response;
           
        }).fail(function (error) {

        });
        return sqlPropertyValue;
    };
    /**
