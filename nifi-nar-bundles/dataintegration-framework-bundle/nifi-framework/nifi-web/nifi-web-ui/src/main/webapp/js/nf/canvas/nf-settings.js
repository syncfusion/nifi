...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['jquery',
                ...
                'nf.ComponentVersion',
                'nf.PolicyManagement',
                'nf.SecurityManagement'],
            function ($, Slick, d3, nfClient, nfDialog, nfCommon, nfCanvasUtils, nfControllerServices, nfErrorHandler, nfReportingTask, nfShell, nfComponentState, nfComponentVersion, nfPolicyManagement,nfSecurityManagement) {
                return (nf.Settings = factory($, Slick, d3, nfClient, nfDialog, nfCommon, nfCanvasUtils, nfControllerServices, nfErrorHandler, nfReportingTask, nfShell, nfComponentState, nfComponentVersion, nfPolicyManagement,nfSecurityManagement));
            });
...
}(this, function ($, Slick, d3, nfClient, nfDialog, nfCommon, nfCanvasUtils, nfControllerServices, nfErrorHandler, nfReportingTask, nfShell, nfComponentState, nfComponentVersion, nfPolicyManagement,nfSecurityManagement) {
    'use strict';
    ...
    var config = {
        urls: {
            ...
            controllerConfig: '../dataintegration-api/controller/config',
            reportingTaskTypes: '../dataintegration-api/flow/reporting-task-types',
            createReportingTask: '../dataintegration-api/controller/reporting-tasks',
            reportingTasks: '../dataintegration-api/flow/reporting-tasks'
        }
    };
    ...
    var matchesRegex = function (item, args) {
        ...
        try {
           ...
           filterExp = new RegExp(args.searchString, 'i');
        } catch (e) {
            // invalid regex
            return false;
        }
    ...
    var addReportingTask = function (reportingTaskType, reportingTaskBundle) {
        ...
        var addTask = $.ajax({
            ...
            var resonseDataURI = reportingTaskEntity.uri;
            var hostName = location.hostname;
            var portNo  = location.port;
            var constructedSecureLink = "https://"+hostName+":"+portNo+"/"+"dataintegration-api";
            resonseDataURI = resonseDataURI.replace(constructedSecureLink, "");
            nf.InheritComponentPolicy.inheritComponentPolicy(resonseDataURI,reportingTaskEntity);   
          }
            // add the item
    ...
    var initReportingTasks = function () {
        ...
        reportingTasksGrid.onClick.subscribe(function (e, args) {
            ...
            if (reportingTasksGrid.getColumns()[args.cell].id === 'actions') {
                if (target.hasClass('edit-reporting-task')) {
                   ...
                   var item = reportingTasksData.getItem(args.row);
                   nfSecurityManagement.ShowSelectedReportingTaskPolicy(item);
                }
                ...
                if (target.hasClass('view-reporting-task')) {
                    ...
                    var selectionData = reportingTaskEntity.component.type;
                    var selectedReportingTask =nfCommon.substringAfterLast(selectionData, '.');
                    // close the settings dialog
                    $('#shell-close-button').click();
                     var url='https://help.syncfusion.com/data-integration/reportingtasks/'+ selectedReportingTask;
        ...
        $('#reporting-tasks-table').data('gridInstance', reportingTasksGrid).on('mouseenter', 'div.slick-cell', function (e) {
            ...
            if (errorIcon.length && !errorIcon.data('qtip')) {
                taskId = $(this).find('span.row-id').text();

                // get the task item
                reportingTaskEntity = reportingTasksData.getItemById(taskId);

                // format the errors
                tooltip = nfCommon.formatUnorderedList(reportingTaskEntity.component.validationErrors);
