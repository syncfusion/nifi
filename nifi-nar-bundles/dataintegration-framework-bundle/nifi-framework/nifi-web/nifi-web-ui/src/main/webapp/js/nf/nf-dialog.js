    ...
    var nfDialog = {
        ...
        showOkDialog: function (options) {
            ...
            $('#nf-ok-dialog').modal('setButtonModel', [{
                ...
                handler: {
                    click: function () {
                        if (options.dialogContent==="Password changed successfully") {
                            window.location = "http://" + window.location.hostname + ":" + window.location.port + "/dataintegration/login";
                        }
                        // close the dialog
