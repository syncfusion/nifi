...
(function (root, factory) {
    ...
    var config = {
        urls: {
            ...
            prioritizers: '../dataintegration-api/flow/prioritizers'
        }
    };
    ...
    var addConnection = function (selectedRelationships) {
        ...
        if (validateSettings()) {
            ...
            $.ajax({
                ...
                //update controller service for PublishDataSource Custom processor
                if((sourceData.component.type)!== undefined && (sourceData.component.type).indexOf("PutSQL") >= 0 && 
                        (destinationData.component.type)!==undefined&&(destinationData.component.type).indexOf("PublishDataSource") >= 0){
                    isValidConnection(sourceData);
                    if(canUpdateConnection)
                        updateDatabaseDetails(sourceData, destinationData);
                }
                
                if((sourceData.component.type)!==undefined&&(sourceData.component.type).indexOf("PutFile") >= 0 && 
                        (destinationData.component.type)!==undefined&&(destinationData.component.type).indexOf("PublishDataSource") >= 0){

                    updateFlatFileDetails(sourceData, destinationData);
                }
    ...
    var isValidConnection = function(sourceData){
        var controllerServiceId = sourceData.component.config.properties["JDBC Connection Pool"];
        canUpdateConnection = false;
        
        $.ajax({
            type: 'GET',
            async: false,
            url: config.urls.api + '/controller-services/' + encodeURIComponent(controllerServiceId),
            contentType: 'application/json'
        }).done(function (response) {
            var connectionUrl = response.component.properties["Database Connection URL"];
            if(connectionUrl === null){
                canUpdateConnection = true;
                return;
            }
            if((response.component.name).indexOf("PostgreSQL")>-1 || (response.component.name).indexOf("Microsoft_SQL_Server")>-1 || 
               (response.component.name).indexOf("MySQL")>-1 || (response.component.name).indexOf("Oracle")>-1){
                canUpdateConnection = true;
                return;
            }
            var connectionList = ["sqlserver", "oracle", "mysql", "postgresql", "sqlite", "hive2"];
            for(var iteration = 0; iteration < connectionList.length; iteration++){
                if(connectionUrl.indexOf(connectionList[iteration])>=0){
                    canUpdateConnection = true;
                    return;
                }
            }
    ...
    var updateDatabaseDetails = function(sourceData, destinationData){
        var processorEntity = {"revision": {
                "version": destinationData.revision.version, 
                "clientId": destinationData.revision.clientId
            }, 
            "id": destinationData.id, 
            "component": {"id": destinationData.id,
                "config": {
                    "properties": {
                        "Connection Type": "Database connection",
                        "Database Connection Pooling Service": sourceData.component.config.properties["JDBC Connection Pool"]
                    }
                }
            }
        };

        updateProcessorEntity(destinationData.id, processorEntity);
    };
    
    var updateFlatFileDetails = function(sourceData, destinationData){
        var processorEntity = {
            "revision": {
                "version": destinationData.revision.version, 
                "clientId": destinationData.revision.clientId
            ...
            "component": {"id": destinationData.id,
                "config": {
                    "properties": {
                        "Flat file location": sourceData.component.config.properties.Directory
                    }
                }
            }
        };
    ...
    var updateProcessorEntity = function(destinationId, processorEntity){
        $.ajax({
            type: 'PUT',
            async: false,
            url: config.urls.api + '/processors/' + encodeURIComponent(destinationId),
            data: JSON.stringify(processorEntity),
            dataType: 'json',
            contentType: 'application/json'
        }).done(function (response) {
            //Nothing to do with response data
        });
    };
    ...
    var initializeDialog = function(source, destination){
    ...
    var nfConnectionConfiguration = {
        ...
        addAvailablePrioritizer: function (prioritizerContainer, prioritizerType) {
            ...
            var prioritizer = $('<li></li>').append($('<span style="float: left;"></span>').text(name)).attr('id', type).addClass('ui-state-default prioritizer-element').appendTo(prioritizerList);
        ...
        createConnection: function (sourceId, destinationId) {
            ...
            if((destinationData.component.type)!==undefined && (destinationData.component.type).indexOf("PublishDataSource")>=0){
                var isDataSourceHasConnection = false;
                
                $.ajax({
                    type: 'GET',
                    async: false,
                    url: config.urls.api + '/process-groups/' + encodeURIComponent(destinationData.component.parentGroupId)+ '/connections',
                    contentType: 'application/json'
                }).done(function (response) {
                    var connectionList = response.connections;
                    for(var iteration = 0; iteration < connectionList.length; iteration++){
                        if(connectionList[iteration].destinationId === destinationData.id){
                            isDataSourceHasConnection = true;
                            break;
                        }
                    }
                    if(!isDataSourceHasConnection)
                        initializeDialog(source, destination);
                    else {
                        var connector = d3.select('path.connector');
                        connector.remove();//Remove connector 
                        
                        nfDialog.showOkDialog({
                            headerText: 'Error',
                            dialogContent: nfCommon.escapeHtml("Destination processor '" + 
                                    destinationData.component.name +"' already have connection.")
                        });
                    }      
                }).fail(function (xhr, status, error){
                    nfDialog.showOkDialog({
                            headerText: 'Error',
                            dialogContent: nfCommon.escapeHtml("Error occurred while getting connection details.")
                        });
                });
            } 
            else 
