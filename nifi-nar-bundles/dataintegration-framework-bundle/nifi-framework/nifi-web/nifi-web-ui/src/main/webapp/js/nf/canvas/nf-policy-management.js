...
(function (root, factory) {
    ...
    var config = {
        urls: {
            ...
            searchTenants: '../dataintegration-api/tenants/search-results'
        }
    };

    var initialized = false;
    var isPolicyBased=false;
    ...
    var initPoliciesDialog = function () {
        $('#policy-management').modal({
            headerText: 'Security',
            handler: {
                close: function () {
                   reset();
                   $("#component-policy-refresh-button").click();
                   $('#global-policy-refresh-button').click();
                }
            }
        });
    };
    ...
    var reset = function () {
        ...
        $("#policy-selected-template-container").hide();
        $("#policy-selected-controller-service-container").hide();
        $("#policy-selected-reporting-task-container").hide();
    ...
    var nfPolicyManagement = {
         ...
         * @param d
         */
        showControllerServicePolicy: function () {
            // reset the policy message
            resetPolicyMessage();
             var canRead= $('#selected-policy-component-permissions').text();
             var componentId=$('#selected-policy-item-id').text();
             var componentName=  $('#selected-policy-item-name').text();
            ...
            if (canRead === "true") {
                $('#policy-selected-controller-service-container div.policy-selected-component-name').text(componentName);
            } else {
                $('#policy-selected-controller-service-container div.policy-selected-component-name').text(componentId);
            }
            $('#policy-selected-controller-service-container').show();

            // populate the initial resource
            $('#selected-policy-component-id').text(componentId);
         ...
         * @param d
         */
        showReportingTaskPolicy: function () {
            // reset the policy message
            resetPolicyMessage();
             var canRead= $('#selected-policy-component-permissions').text();
             var componentId=$('#selected-policy-item-id').text();
            ...
            // update the visibility
            if (canRead === "true") {
                $('#policy-selected-reporting-task-container div.policy-selected-component-name').text(componentName);
            } else {
                $('#policy-selected-reporting-task-container div.policy-selected-component-name').text(componentId);
            }
        ...
        showComponentPolicy: function (selection) {
            ...
            var resource;
            if($('#selected-policy-item-type').text()==='controller-services')
            {
              $('#selected-component-policy-id').text($('#selected-policy-item-id').text());
                resource = 'controller-services';
            }
            else if($('#selected-policy-item-type').text()==='reporting-tasks')
            {
              $('#selected-component-policy-id').text($('#selected-policy-item-id').text());
                resource = 'reporting-tasks';
            }
        else
        {
            if (selection.empty()) {
