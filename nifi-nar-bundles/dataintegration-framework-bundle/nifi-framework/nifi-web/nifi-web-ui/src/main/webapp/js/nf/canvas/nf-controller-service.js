...
(function (root, factory) {
    ...
    var updateSQLProperty = function (updatedControllerService) {
        if (nfCommon.isDefinedAndNotNull(updatedControllerService)) {
            if(updatedControllerService.component.properties !== undefined){
                if (updatedControllerService.component.name === "MySQL"){
                    if(updatedControllerService.component.properties["MySQL-database-driver-locations"] !== undefined)
                        updateSQLPropertyFile("MySQL", updatedControllerService.component.properties["MySQL-database-driver-locations"]);
                } else if(updatedControllerService.component.name === "PostgreSQL"){
                    if(updatedControllerService.component.properties["PostgreSQL-database-driver-locations"] !== undefined)
                        updateSQLPropertyFile("PostgreSQL", updatedControllerService.component.properties["PostgreSQL-database-driver-locations"]);
                }else if(updatedControllerService.component.name === "Oracle"){
                    if(updatedControllerService.component.properties["Oracle-database-driver-locations"] !== undefined)
                        updateSQLPropertyFile("Oracle", updatedControllerService.component.properties["Oracle-database-driver-locations"]);
                }else if(updatedControllerService.component.name === "MicrosoftSQLServer"){
                    if(updatedControllerService.component.properties["MicrosoftSQL-database-driver-locations"] !== undefined)
                        updateSQLPropertyFile("SQLServer", updatedControllerService.component.properties["MicrosoftSQL-database-driver-locations"]);
                }
            }
        }
    };
    
    var updateSQLPropertyFile = function (databasetype, jarFileName) {
        var jarFileArray = jarFileName.split("\\");
        jarFileName = jarFileArray.join("\\\\");
    ...
    var reloadControllerServiceReferences = function (serviceTable, controllerService) {
        ...
        $.each(controllerService.referencingComponents, function (_, referencingComponentEntity) {
            ...
            if (reference.referenceType === 'Processor') {
                ...
                referencingComponentState = $('div.' + reference.id + '-state');
    ...
    var getControllerService = function (controllerServiceId, controllerServiceData) {
        ...
        if (nfCommon.isDefinedAndNotNull(controllerServiceEntity)) {
            ...
            return $.ajax({
                type: 'GET',
                url: '../dataintegration-api/controller-services/' + encodeURIComponent(controllerServiceId),
                dataType: 'json'
            }).fail(nfErrorHandler.handleAjaxError);
        }
    };
    ...
    var saveControllerService = function (serviceTable, controllerServiceEntity) {
        ...
        if (validateDetails(updatedControllerService)) {
            ...
            return $.ajax({
                ...
                updateSQLProperty(updatedControllerService);
    ...
    var nfControllerService = {
        ...
        controllerServiceUrl:undefined,
        /**
        ...
        showConfiguration: function (serviceTable, controllerServiceEntity) {
            nfControllerService.controllerServiceUrl = controllerServiceEntity.uri;
            ...
            var loadHistory = $.ajax({
                type: 'GET',
                url: '../dataintegration-api/flow/history/components/' + encodeURIComponent(controllerServiceEntity.id),
                dataType: 'json'
            });
                ...
                var buttons = [{
                    ...
                    handler: {
                            click: function () {
                                ...
                                // reload the service in case the property descriptors have changed
                                var updatedControllerServiceEntity = $.ajax({
                                    type: 'GET',
                                    url: controllerServiceEntity.uri,
                                    dataType: 'json'
                                });
                                $.when(updatedControllerServiceEntity).done(function (updatedControllerServiceEntity) {
                                    controllerServiceEntity = updatedControllerServiceEntity;
