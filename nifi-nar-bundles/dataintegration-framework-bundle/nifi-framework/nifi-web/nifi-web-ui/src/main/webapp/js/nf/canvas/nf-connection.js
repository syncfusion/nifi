...
(function (root, factory) {
 ...
 /**
     ...
     * @argument {string} type      The port type
     */
    var isFunnelType = function (type) {
        return type.indexOf('FUNNEL') >= 0;
    };
 ...
 /**
     ...
     * @param {object} terminal
     */
    var isPort = function (terminal) {
        return terminal.groupId === nfCanvasUtils.getGroupId() && (isInputPortType(terminal.type) || isOutputPortType(terminal.type)|| isFunnelType(terminal.type));
    };
    /**
     * /**
     * Determines whether the terminal of the connection (source|destination) is
     ...
     * @param {object} terminal
     */
    var isProcessor = function (terminal) {
        return terminal.type.indexOf('PROCESSOR') >= 0;
    };
    /**
    ...
    var renderConnections = function (entered, selected) {
        ...
        var connection = entered.append('g')
            .attr({
                ...
                'class': 'connection  minimized-connector'
            })
    ...
    // determines whether the connection count is greater than zero
    var hasCount = function (d) {
        return parseInt(d.status.aggregateSnapshot.queuedCount)>0;
    };
 ...
 // determines whether the connection data size is greater than zero
    var hasBytes = function (d) {
        return parseInt(nfCommon.substringBeforeFirst(d.status.aggregateSnapshot.queued, ' '))> 0;
    };
      ...
      updated.each(function(d) {
          ...
          if (updatePath === true) {
              ...
              if (d.bends.length > 0) {
                  ...
                  if ($("#id-" + sourceComponentId).hasClass("minimized")|| $("#id-" + sourceComponentId).hasClass("minimized-process-group")) {
                      endAnchor = {
                          x: sourceData.position.x + (sourceData.minimizedDimensions.width / 2),
                          y: sourceData.position.y + (sourceData.minimizedDimensions.height / 2)
                      };
                  } else {
                      endAnchor = {
              ...
              if (nfCommon.isDefinedAndNotNull(d.end) && d.end.dragging === true) {
                  ...
                  if (!newDestination.empty()) {
                      ...
                      var newDestinationWidth;
                      var newDestinationHeight;
                      if ($("#id-" + newDestination).hasClass("minimized")|| $("#id-" + newDestination).hasClass("minimized-process-group")) {
                          newDestinationWidth = newDestinationData.minimizedDimensions.width;
                          newDestinationHeight = newDestinationData.minimizedDimensions.height;
                      } else {
                          newDestinationWidth = newDestinationData.dimensions.width;
                          newDestinationHeight = newDestinationData.dimensions.height;
                      }
                      ...
                      var newEnd = nfCanvasUtils.getPerimeterPoint(endAnchor, {
                          ...
                          'width': newDestinationWidth,
                          'height': newDestinationHeight
                      });
                  ...
                  var destinationComponentWidth;
                  var destinationComponentWidth;
                  if ($("#id-" + destinationComponentId).hasClass("minimized")||$("#id-" + destinationComponentId).hasClass("minimized-process-group")) {
                      destinationComponentWidth = destinationData.minimizedDimensions.width;
                      destinationComponentHeight = destinationData.minimizedDimensions.height;
                  } else {
                      destinationComponentWidth = destinationData.dimensions.width;
                      destinationComponentHeight = destinationData.dimensions.height;
                  }
                  ...
                  // get the position on the destination perimeter
                      ...
                      'width': destinationComponentWidth,
                      'height': destinationComponentHeight
                  });
              }
          ...
          if (updateLabel === true) {
              ...
              if (connection.classed('visible')) {
                  ...
                  if (connectionLabelContainer.empty()) {
                      ...
                      connectionLabelContainer.append('rect')
                          .attr({
                              'class': 'border maximize-connector',
                              'width': dimensions.width,
                              'fill': 'transparent',
                              'stroke': 'transparent'
                          });
                  ...
                  if (d.permissions.canRead) {
                      ...
                      if (isGroup(d.component.source)) {
                          ...
                          if (connectionFrom.empty()) {
                              ...
                              backgrounds.push(connectionFrom.append('rect')
                                  .attr({
                                      'class': 'connection-label-background maximize-connector',
                                      'width': 118,
                                      'height': 1
                                  }));
                              ...
                              borders.push(connectionFrom.append('rect')
                                  .attr({
                                      'class': 'connection-label-border maximize-connector',
                                      'width': 118,
                                      'height': 1
                                  }));
                              ...
                              connectionFrom.append('text')
                                  .attr({
                                      'class': 'stats-label maximize-connector',
                                      'x': 5,
                                      'y': 14
                                  })
                                  .text('From');
                              ...
                              connectionFrom.append('text')
                                  .attr({
                                      'class': 'stats-value connection-from maximize-connector',
                                      'x': 43,
                                      'y': 14,
                                      'width':30
                                  });
                              ...
                              connectionFrom.append('text')
                                  .attr({
                                      'class': 'connection-from-run-status maximize-connector',
                                      'x': 85,
                                      'y': 14
                                  });
                              connectionFrom.append('image')
                                  .attr({
                                      ...
                                      'height': 14,
                                      'class': 'connector-arrow-icon maximize-connector'
                                  });
                      ...
                      if (isGroup(d.component.destination)) {
                        var connectionNameValue = nfCanvasUtils.formatConnectionName(d.component);
                          ...
                          if (connectionTo.empty()) {
                              ...
                              borders.push(connectionTo.append('rect')
                                  .attr({
                                      'class': 'connection-label-border connection-label-border maximize-connector',
                                      'width': 118,
                                      'height': 1
                                  }));
                              ...
                              connectionTo.append('text')
                                  .attr({
                                      'class': 'stats-value connection-to maximize-connector',
                                      'x': 25,
                                      'y': 14,
                                      'width': 30
                                  });
                             connectionTo.append('circle')
                             .attr({
                              'class': 'connection-circle minimize-connector',
                              'r': 5,
                              'fill': '#ffffff',
                              'stroke': function(d) {
                                  return "#FF9E1D";
                              ...
                              'cx': 60,
                              'cy':function(d){
                                   if(isProcessor(d.component.source) &&(isGroup(d.component.destination)) ){
                                       return '28';
                                   }
                                    else if((isPort(d.component.source)&&!nfCommon.isBlank(connectionNameValue))&&(isGroup(d.component.destination))){
                                       return '28';
                                   }
                                    else if((isGroup(d.component.destination)&&!nfCommon.isBlank(connectionNameValue)) && isGroup(d.component.source)){
                                       return '20';
                                   }
                                   else if(isGroup(d.component.destination) && isGroup(d.component.source)){
                                       return '11';
                                   }
                                   else{
                                     return '20';  
                              ...
                              connectionTo.append('text')
                                  .attr({
                                      'class': 'connection-to-run-status maximize-connector',
                                      'x': 85,
                                      'y': 14
                                  });
                               if(isPort(d.component.source)||(isGroup(d.component.destination)&&!isGroup(d.component.source))){
                           ...
                           connectionTo.select('circle.connection-circle')
                           .attr('cy', function (d) {
                               if((isProcessor(d.component.source)||isPort(d.component.source))&&(isGroup(d.component.destination))){
                                       return '28';
                                   }                        
                                   else if(isGroup(d.component.destination) && isGroup(d.component.source)){
                                       return '20';
                        ...
                        connectionTo.select('circle.connection-circle')
                           .attr('cy', function (d) {
                                if(isGroup(d.component.destination) && isGroup(d.component.source)){
                                       return '11';
                                   }
                                   else if((isProcessor(d.component.source))&&(isGroup(d.component.destination))){
                                       return '28';
                                   }                        
                                   else{
                                     return '20';  
                      ...
                      if (!nfCommon.isBlank(connectionNameValue)) {
                          ...
                          if (connectionName.empty()) {
                              connectionName = connectionLabelContainer.append('g')
                                  .attr({
                                      'class': 'connection-name-container maximize-connector'
                                  });
                  ...
                  if (queued.empty()) {
                    ...
                    if(isPort(d.component.source) && (isPort(d.component.destination)||isProcessor(d.component.destination))){
                      // get the connection name
                      var connectionNameValue = nfCanvasUtils.formatConnectionName(d.component);
                        if(nfCommon.isBlank(connectionNameValue)){
                         queued.append('image')
                    ...
                    if (!isGroup(d.component.destination)) {
                      queued.append('circle')
                          .attr({
                              ...
                              'cx': 60,
                              'cy':function(d){                        
                                  if(nfCommon.isBlank(connectionNameValue) &&(!isPort(d.component.source)) ){
                                       return '2';
                                   }
                                   else if(!nfCommon.isBlank(connectionNameValue)&&((isProcessor(d.component.destination)||isPort(d.component.destination))&& isGroup(d.component.source))){
                                      return '-8'; 
                                   }
                                   else if(!nfCommon.isBlank(connectionNameValue)){
                                      return '2'; 
                                   }
                                   else
                                     return '11'; 
                      ...
                      queued.append('text')
                          .attr({
                              'class': 'stats-value queued queued-text maximize-connector',
                              'x': 66,
                              'y': 14,
                              'width': 40
                          });
                      ...
                      queued.append('text')
                          .attr({
                              'class': 'expiration-icon maximize-connector',
                              'x': 53,
                              'y': 15
                          })
                          .text(function() {
                      ...
                      queued.append('rect')
                          .attr({
                              'class': 'backpressure-tick object maximize-connector',
                              'width': 1,
                              'height': 3,
                              'x': 5,
                      ...
                      queued.append('rect')
                          .attr({
                              'class': 'backpressure-object maximize-connector',
                      ...
                      queued.append('rect')
                          .attr({
                              'class': 'backpressure-percent object maximize-connector',
                              'width': 0,
                              'height': 3,
                              'x': backpressureCountOffset,
                              'y': yBackpressureOffset
                      ...
                      queued.append('rect')
                          .attr({
                              'class': 'backpressure-tick data-size maximize-connector',
                              'width': 1,
                              'height': 3,
                      ...
                      queued.append('rect')
                          .attr({
                              'class': 'backpressure-data-size maximize-connector',
                              'width': backpressureBarWidth,
                              'height': 3,
                      ...
                      queued.append('rect')
                          .attr({
                              'class': 'backpressure-percent data-size maximize-connector',
                              'width': 0,
                              'height': 3,
                              'x': backpressureDataSizeOffset,
                              'y': yBackpressureOffset
                       ...
                       var connectionNameValue = nfCanvasUtils.formatConnectionName(d.component);
                      if(!nfCommon.isBlank(connectionNameValue)){
                          queued.select('image.connector-arrow-icon').remove();
                          queued.select('circle.connection-circle')
                           .attr('cy', function (d) {
                                   if((isProcessor(d.component.destination)||isPort(d.component.destination))&& isGroup(d.component.source)){
                                      return '-8'; 
                                   }
                                   else if(!nfCommon.isBlank(connectionNameValue)){
                                      return '2'; 
                         ...
                         queued.select('circle.connection-circle')
                           .attr('cy', function (d) {
                                  if(!isPort(d.component.source)){
                                       return '2';
                                   }
                                   else if((isProcessor(d.component.destination)||isPort(d.component.destination))&& isGroup(d.component.source)){
                                      return '2'; 
                                   }
                                   else
                                     return '11'; 
                               });  
                               if((!isGroup(d.component.source)) && (!isGroup(d.component.destination)))
                               {
                                var arrow=queued.select('image.connector-arrow-icon');
                                 if(arrow.empty()){
                                 queued.append('image')
                                  .attr({
                                      'x': 103,
          ...
          if ($("#" + connectionId).hasClass("minimized-connector")) {
              
              $("#" + connectionId).find(".maximize-connector").attr("visibility","hidden");
              $("#" + connectionId).find(".minimize-connector").attr("visibility","visible");

          } else {
              $("#" + connectionId).find(".maximize-connector").attr("visibility","visible");
              $("#" + connectionId).find(".minimize-connector").attr("visibility","hidden");
          }
  ...
  $(document).on("click", ".connection-circle", function() {
      var selectedConnection = d3.select(this.parentElement.parentElement.parentElement);
      var connectionId = selectedConnection.attr("id");
      $("#" + connectionId).find(".maximize-connector").attr("visibility","visible");
      $("#" + connectionId).removeClass("minimized-connector");
      $("#" + connectionId).addClass("maximized-connector");
      $("#" + connectionId).find(".minimize-connector").attr("visibility","hidden");
  });
  $(document).on("click", ".connector-arrow-icon", function() {
      var selectedConnection = d3.select(this.parentElement.parentElement.parentElement);
      var connectionId = selectedConnection.attr("id");
      $("#" + connectionId).find(".maximize-connector").attr("visibility","hidden");
      $("#" + connectionId).find(".minimize-connector").attr("visibility","visible");
      $("#" + connectionId).removeClass("maximized-connector");
      $("#" + connectionId).addClass("minimized-connector");
  });
    /**
    ...
    var updateConnectionStatus = function (updated) {
        ...
        var dataSizeDeferred = $.Deferred(function (deferred) {
            // queued count value
              ...
              if($("#"+id).find(".queued-text").css("display")==="none"){
               updated.select('text.queued-text')
                .text(function (d) {
                    return nfCommon.substringBeforeFirst(d.status.aggregateSnapshot.queued, ' ')+ ' ' + nfCommon.substringAfterFirst(d.status.aggregateSnapshot.queued, ' ');
                });   
              }
              else{
                  updated.select('.queued-text').each(function (d) {
                            var queuedLabel = d3.select(this);
                                    // reset the label name to handle any previous state
                              queuedLabel.text(null).selectAll('title').remove();
                                    // apply ellipsis to the label as necessary
                               nfCanvasUtils.ellipsis(queuedLabel, d.status.aggregateSnapshot.queued);
                                }).append('title').text(function (d) {
                                return d.status.aggregateSnapshot.queued;
                            }); 
              }
        ...
        $.when(dataSizeDeferred, objectCountDeferred).done(function () {
            ...
            updated.select('path.connection-path')
              ...
              updated.select('.connection-circle')
                      .attr('fill', function (d) {
                           return hasCount(d) || hasBytes(d)?'#FF9E1D':'#ffffff';
                        });
            // border
    ...
    var nfConnection = {
        ...
        reloadStatus: function (id) {
            if (connectionMap.has(id)) {
                return $.ajax({
                    type: 'GET',
                    url: '../dataintegration-api/flow/connections/' + encodeURIComponent(id) + '/status',
                    dataType: 'json'
                }).done(function (response) {
