...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['jquery',
                ...
                'nf.ng.Bridge',
               'nf.Connection'],
            function ($, d3, nfCommon, nfCanvasUtils, nfNgBridge,nfConnection) {
                return (nf.ContextMenu = factory($, d3, nfCommon, nfCanvasUtils, nfNgBridge,nfConnection));
            });
...
}(this, function ($, d3, nfCommon, nfCanvasUtils, nfNgBridge,nfConnection) {
    'use strict';
    ...
    /* * Determines whether the components in the specified selection are publishable.
     * 
     * @param {selection} selection         The selection of currently selected components
     */
      var canPublishTemplate = function (selection) {
        return nfCanvasUtils.canPublish(selection);
    };
     /* 
    ...
    var canAccessProvenance = function (selection) {
            // ensure the correct number of components are selected
            if (selection.size() >= 1) {
                return !nfCanvasUtils.isLabel(selection) && !nfCanvasUtils.isConnection(selection) && !nfCanvasUtils.isRemoteProcessGroup(selection) && nfCommon.canAccessProvenance();
            }        
    };
    /**
     * Determines whether the current selection could have data to view.
     *
     * @param {selection} selection
     */
    var canAccessDataPreview = function (selection) {
        if (selection.size() === 1) {
    ...
    if (connectioncount === selection[0].length || processgroupcount === selection[0].length) {
        return true;
    } else {
        return false;
    }
    ...
    var canMoveToParent = function (selection) {
        ...
        var count = 0;
        var movetoparent = true;
        for (count; count < selection[0].length; count++) {
            var selectedcomponentid = selection[0][count].id;
            var selectedcomponentclass = $("#" + selectedcomponentid).attr("class");
            if (selectedcomponentclass.indexOf("process-group") === -1) {
                if ($("#data-flow-title-container .link").length < 3) {
                    movetoparent = false;
                    break;
                } else {
                    movetoparent = true;
     ...
     return !selection.empty()&& nfConnection.isDisconnected(selection)&& nfCanvasUtils.getParentGroupId() !== null &&movetoparent===true ;
    };
    ...
    var addMenuItem = function (contextMenu, item) {
        ...
        if(window.location.protocol === "http:"){            
            if(localStorage.getItem(window.location.hostname+"-status") === null || localStorage.getItem(window.location.hostname+"-status") === "false")
                if(item.text === "Configure" || item.text === "Group" || item.text === "Start" || item.text === "Copy" || item.text === "Paste" ||
                        item.text === "Stop" || item.text === "Delete" || item.text === "Template" || item.text==="Empty queue")
                    className = "disable-element";           
        }
        var menuItem = $('<div class="context-menu-item '+className+'"></div>').attr('id', item.id).on('mouseenter', function () {
            $(this).addClass('hover');
    ...
    var menuItems = [
        ...
        {separator: true},
        {id: 'show-configuration-menu-item', condition: isConfigurable, menuItem: {clazz: 'context-menu-items icon-configure', text: 'Configure', action: 'showConfiguration'}},
        {id:'preview-data',condition: canAccessDataPreview, menuItem: {clazz: 'icon icon-preview', text: 'Data Preview', action: 'showDataPreview'}},
        {id: 'show-details-menu-item', condition: hasDetails, menuItem: {clazz: 'fa fa-gear', text: 'View configuration', action: 'showDetails'}},
        {id: 'variable-registry-menu-item', condition: hasVariables, menuItem: {clazz: 'context-menu-items icon-variables', text: 'Variables', action: 'openVariableRegistry'}},
        {separator: true},
        {id: 'enter-group-menu-item', condition: isProcessGroup, menuItem: {clazz: 'context-menu-items icon-Entergroup', text: 'Enter group', action: 'enterGroup'}},
        {separator: true},
        ...
        {id: 'view-menu-items', groupMenuItem: {clazz: 'context-menu-items icon-view', text: 'View'}, menuItems: [
            {id: 'data-provenance-menu-item', condition: canAccessProvenance, menuItem: {clazz: 'icon icon-provenance', imgStyle: 'context-menu-provenance', text: 'Data provenance', action: 'openProvenance'}},
            {id: 'show-stats-menu-item', condition: supportsStats, menuItem: {clazz: 'context-menu-items icon-StatusHistory', text: 'Status history', action: 'showStats'}},
            {id: 'view-state-menu-item', condition: isStatefulProcessor, menuItem: {clazz: 'fa fa-tasks', text: 'State', action: 'viewState'}},        
            {id: 'show-usage-menu-item', condition: hasUsage, menuItem: {clazz: 'fa fa-book', text: 'Usage', action: 'showUsage'}},
            {id: 'view-menu-item', groupMenuItem: {clazz: 'icon icon-connect', text: 'Connections'}, menuItems: [
                {id: 'show-upstream-menu-item', condition: hasUpstream, menuItem: {clazz: 'context-menu-items icon-UpstreamConnection', text: 'Upstream', action: 'showUpstream'}},
                {id: 'show-downstream-menu-item', condition: hasDownstream, menuItem: {clazz: 'context-menu-items icon-DownstreamConnections', text: 'Downstream', action: 'showDownstream'}}
            ]}
        ]},
        {separator: true},
        ...
        {id: 'manage-policies-menu-item', condition: canManagePolicies, menuItem: {clazz: 'fa fa-lock', text: 'Manage access policies', action: 'managePermissions'}},
        ...
        {separator: true},
        {id: 'align-menu-item', groupMenuItem: {clazz: 'fa fa-align-justify', text: 'Align'}, menuItems: [
        ...
        {id: 'center-menu-item', condition: isNotConnection, menuItem: {clazz: 'context-menu-items icon-Centerinview', text: 'Center in view', action: 'center'}},
        ...
        {id: 'group-menu-item', condition: canGroup, menuItem: {clazz: 'context-menu-items icon-Group', text: 'Group', action: 'group'}},
        {separator: true},
        {id: 'template-menu-items', groupMenuItem: {clazz: 'icon icons-tab_templetes', text: 'Template'}, menuItems: [
            {id: 'upload-template-menu-item', condition: canUploadTemplate, menuItem: {clazz: 'icon icon-template-import', text: 'Upload', action: 'uploadTemplate'}},
            {id: 'publish-template-menu-item', condition: canPublishTemplate, menuItem: {clazz: 'context-menu-items icon-Publish', text: 'Publish', action: 'publishTemplate'}},
            {id: 'template-menu-item', condition: canCreateTemplate, menuItem: {clazz: 'icon icon-template-save', text: 'Create', action: 'template'}
        }]},
        {separator: true},
        {id: 'copy-menu-item', condition: isCopyable, menuItem: {clazz: 'context-menu-items icon-Copy', text: 'Copy', action: 'copy'}},
        {id: 'paste-menu-item', condition: isPastable, menuItem: {clazz: 'fa fa-paste', text: 'Paste', action: 'paste'}},
        {separator: true},
        {id: 'empty-queue-menu-item', condition: canEmptyQueue, menuItem: {clazz: 'context-menu-items fa fa-minus-circle', text: 'Empty queue', action: 'emptyQueue'}},
        {id: 'delete-menu-item', condition: isDeletable, menuItem: {clazz: 'context-menu-items icon-Delete', text: 'Delete', action: 'delete'}}	
    ];
