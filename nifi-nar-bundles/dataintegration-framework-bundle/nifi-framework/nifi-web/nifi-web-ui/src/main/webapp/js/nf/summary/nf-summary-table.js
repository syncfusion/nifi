...
(function (root, factory) {
    ...
    var config = {
        urls: {
            ...
            status: '../dataintegration-api/flow/process-groups/root/status',
            systemDiagnostics: '../dataintegration-api/system-diagnostics',
            flowConfig: '../dataintegration-api/flow/config'
        }
    };
    ...
    var initSummaryTable = function (isClustered) {
        ...
        var processorGroupMoreDetails = function (row, cell, value, columnDef, dataContext) {
            var markup = '<div title="View Processor group Details" class="pointer show-processor-group-details fa fa-info-circle" style="margin-right: 3px;"></div>';
        ...
        var processorGroupMoreDetailsColumn = {
            id: 'moreDetails',
            field: 'moreDetails',
            name: '&nbsp;',
            resizable: false,
            formatter: processorGroupMoreDetails,
            sortable: true,
            width: 50,
            maxWidth: 50,
            toolTip: 'Sorts based on presence of bulletins'
        ...
        var processGroupsColumnModel = [
            processorGroupMoreDetailsColumn,
            {id: 'name', field: 'name', name: 'Name', sortable: true, resizable: true, formatter: valueFormatter},
            transferredColumn,
            inputColumn,
            ioColumn,
        ...
        processGroupsGrid.onClick.subscribe(function (e, args) {
            ...
            } else if (processGroupsGrid.getColumns()[args.cell].id === 'moreDetails') {
                if (target.hasClass('show-processor-group-details')) {
                    nfProcessorDetails.showProcessorGroupDetails(item.id, item.name);
                }
            }
        });
    ...
    var sort = function (tableId, sortDetails, data) {
        ...
        var comparer = function (a, b) {
            ...
            if (sortDetails.columnId === 'moreDetails') {
                ...
                aString = nfCommon.isDefinedAndNotNull(a[sortDetails.columnId]) ? a[sortDetails.columnId] : '';
                bString = nfCommon.isDefinedAndNotNull(b[sortDetails.columnId]) ? b[sortDetails.columnId] : '';
                if (aString === bString) {
