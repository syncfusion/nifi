...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['jquery',
                ...
                'nf.ComponentVersion',
                'nf.ng.Bridge'],
            function ($, d3, Slick, nfClient, nfShell, nfProcessGroupConfiguration, nfCanvasUtils, nfErrorHandler, nfDialog, nfCommon, nfControllerService, nfProcessGroup, nfPolicyManagement, nfComponentState, nfComponentVersion, nfNgBridge,nfSecurityManagement,nfComponentPolicy) {
                return (nf.ControllerServices = factory($, d3, Slick, nfClient, nfShell, nfProcessGroupConfiguration, nfCanvasUtils, nfErrorHandler, nfDialog, nfCommon, nfControllerService, nfProcessGroup, nfPolicyManagement, nfComponentState, nfComponentVersion, nfNgBridge,nfSecurityManagement,nfComponentPolicy));
            });
        ...
        module.exports = (nf.ControllerServices =
            factory(require('jquery'),
                ...
                require('nf.SecurityManagement'),
                require('nf.ComponentPolicy')));
    } else {
        nf.ControllerServices = factory(root.$,
            ...
            root.nf.ng.Bridge,
            root.nf.SecurityManagement,
            root.nf.nfComponentPolicy);
    }
}(this, function ($, d3, Slick, nfClient, nfShell, nfProcessGroupConfiguration, nfCanvasUtils, nfErrorHandler, nfDialog, nfCommon, nfControllerService, nfProcessGroup, nfPolicyManagement, nfComponentState, nfComponentVersion, nfNgBridge,nfSecurityManagement,nfComponentPolicy) {
    'use strict';
    ...
    var config = {
        urls: {
            ...
            controllerServiceTypes: '../dataintegration-api/flow/controller-service-types'
        }
    };
    ...
    var addControllerService = function (controllerServicesUri, serviceTable, controllerServiceType, controllerServiceBundle) {
        ...
        var addService = $.ajax({
           ...
           if(location.protocol !== "http:"){
            var responseDataURI = controllerServiceEntity.uri;
            var hostName = location.hostname;
            var portNo  = location.port;
            var constructedSecureURI = "https://"+hostName+":"+portNo+"/"+"dataintegration-api";
            responseDataURI = responseDataURI.replace(constructedSecureURI, "");
            nf.InheritComponentPolicy.inheritComponentPolicy(responseDataURI,controllerServiceEntity);   
          }
            // add the item
    ...
    var initNewControllerServiceDialog = function () {
        ...
        $('#controller-service-types-table').data('gridInstance', controllerServiceTypesGrid).on('mouseenter', 'div.slick-cell', function (e) {
            ...
            if (usageRestriction.length && !usageRestriction.data('qtip')) {
                rowId = $(this).find('span.row-id').text();

                // get the status item
                item = controllerServiceTypesData.getItemById(rowId);
    ...
    var initControllerServices = function (serviceTable, showSettings) {
        ...
        var controllerServiceStateFormatter = function (row, cell, value, columnDef, dataContext) {
            ...
            if (!nfCommon.isEmpty(dataContext.component.validationErrors)) {
                ...
                if (dataContext.component.state === 'DISABLED') {
                    icon = 'disabled icon icon-enable-controller-false"';
                    label = 'Disabled';
        ...
        var controllerServiceActionFormatter = function (row, cell, value, columnDef, dataContext) {
            ...
            if (nfCanvasUtils.isManagedAuthorizer() && nfCommon.canAccessTenants() && dataContext.permissions.canRead) {
        ...
        controllerServicesGrid.onClick.subscribe(function (e, args) {
            ...
            controllerServiceEntity = controllerServicesData.getItem(args.row);
            ...
            if (controllerServicesGrid.getColumns()[args.cell].id === 'actions') {
                if (target.hasClass('edit-controller-service')) {
                  ...
                  // close the settings dialog
                   ...
                   // nfPolicyManagement.showControllerServicePolicy(controllerServiceEntity);
                    var item = controllerServicesData.getItem(args.row);
                    nfSecurityManagement.ShowSelectedControllerServicePolicy(item);
                    // close the settings dialog
                  //  $('#shell-close-button').click();
                ...
                if (target.hasClass('controller-service-usage')) {
                    var selectionData = controllerServiceEntity.component.type;
                    var selectedControllerService =nfCommon.substringAfterLast(selectionData, '.');
                     // close the settings dialog
                     $('#shell-close-button').click();
                     var url='https://help.syncfusion.com/data-integration/controllerservices/' + selectedControllerService;
        ...
        serviceTable.data('gridInstance', controllerServicesGrid).on('mouseenter', 'div.slick-cell', function (e) {
            ...
            if (errorIcon.length && !errorIcon.data('qtip')) {
                ...
                controllerServiceEntity = controllerServicesData.getItemById(serviceId);

                // format the errors
                tooltip = nfCommon.formatUnorderedList(controllerServiceEntity.component.validationErrors);
            ...
            if (bulletinIcon.length && !bulletinIcon.data('qtip')) {
                ...
                // get the task item
                controllerServiceEntity = controllerServicesData.getItemById(taskId);

                // format the tooltip
                var bulletins = nfCommon.getFormattedBulletins(controllerServiceEntity.bulletins);
                tooltip = nfCommon.formatUnorderedList(bulletins);
