    ...
    return {
        ...
        showConfiguration: function (selection) {
            if (nfCanvasUtils.isProcessor(selection)) {
                ...
                requests.push($.ajax({
                    type: 'GET',
                    url: '../dataintegration-api/flow/history/components/' + encodeURIComponent(processor.id),
                    dataType: 'json'
                }));
                ...
                $.when.apply(window, requests).done(function (processorResult, historyResult) {
                    ...
                    //add custom url
                    if (processor.type.indexOf("linkedIn") > -1 || processor.type.indexOf("Instagram") > -1 || processor.type.indexOf("Facebook") > -1) {
                        var processorType = processor.type.split(".");
                        var type = processorType[processorType.length - 1];
                        var url = "";
                        var label = "";
                        if (type === "GetLinkedIn" || type === "PostLinkedIn") {
                            url = "https://developer.linkedin.com/";
                            label = "LinkedIn";
                           }
                          else if(type==="GetInstagram"){
                            url = "https://www.instagram.com/developer/endpoints/";
                            label = "Instagram";
                           }
                          else if(type==="GetFacebook"){
                            url = "https://developers.facebook.com/docs/graph-api/reference";
                            label = "Facebook";                              
                           } 
                         $('#processor-configuration .dialog-buttons').append('<div class="custom-url"><span class="start-element">To learn about '+label+' endpoints, go</span><a href="' + url + '" target="_blank" class="endpoint-url">here</a></div>');
                    }
