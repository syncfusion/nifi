        ...
        handleAjaxError: function (xhr, status, error) {
            ...
            if ($('#splash').is(':visible')) {
                if (xhr.status === 401) {
                    ...
                    $('#message-title').text('Unauthorized Access');
            ...
            if (xhr.status === 400 || xhr.status === 404 || xhr.status === 409 || xhr.status === 503) {
                var dialogContent;
                if (xhr.responseText.indexOf('Queue not empty for') > -1) {
                    dialogContent='Queue is not empty for the selected components. Empty the queue first and try again.';
                    } 
                    else if(xhr.responseText.indexOf('The Templates must be deleted first') > -1){
                    dialogContent=xhr.responseText;
                    }
                    else {
                ...
                nfDialog.showOkDialog({
                    ...
                    okHandler: function () {
                        if(nfCommon.escapeHtml(xhr.responseText).indexOf("Unable to locate template")>-1)
                             addTemplate(false);
                        }
                });
            } else if (xhr.status === 403) {
                ...
                if (xhr.status < 99 || xhr.status === 12007 || xhr.status === 12029) {
                    ...
                    $('#message-title').text('Unable to communicate with Data Integration Platform');
                    $('#message-content').text(content);
                    if(status !== 'timeout' || status !== 'abort' || status !== 'No Transport'){
                        $('#message-pane').hide();
                        checkStatus = setInterval(checkServiceStatus, 3000);
                        canUpdateStatus = false;
                    }
                } else if (xhr.status === 401) {
                    $('#message-title').text('Unauthorized');
                    if ($.trim(xhr.responseText) === '') {
                        $('#message-content').text('Authentication is required to use this Data Integration Platform.');
                    } else {
                        $('#message-content').text(xhr.responseText);
                    }
                    ...
                    if ($.trim(xhr.responseText) === '') {
                        $('#message-content').text('Unable to interpret response from Data Integration Platform.');
                    } else {
                        $('#message-content').text(xhr.responseText);
                    }
                } else {
