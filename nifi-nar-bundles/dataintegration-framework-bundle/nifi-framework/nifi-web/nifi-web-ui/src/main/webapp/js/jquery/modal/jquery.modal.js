...
(function ($) {
    ...
    var addButtons = function (dialog, buttonModel) {
        if (isDefinedAndNotNull(buttonModel)) {
         ...
         if (isDefinedAndNotNull(buttonConfig.color)) {
                    if (buttonConfig.buttonText === "Add" || buttonConfig.buttonText === "Ok" || buttonConfig.buttonText === "Apply" ||
                            buttonConfig.buttonText === "Empty" ||
                            buttonConfig.buttonText === "Filter" || buttonConfig.buttonText === "Create" ||
                            buttonConfig.buttonText === "Close" || buttonConfig.buttonText === "Stop" ||
                            buttonConfig.buttonText === "Search" || buttonConfig.buttonText === "Purge" ||
                            buttonConfig.buttonText === "Yes" || buttonConfig.buttonText === "Enable" ||
                            buttonConfig.buttonText === "Disable" ||
                            buttonConfig.buttonText === "VIEW")
                    {
                        button.css({
                            'background': '#448DD5',
                            'color': '#fff',
                    ...
                    else if (buttonConfig.buttonText === "Advanced")
                    {
                        button.css({
                            'background': '#448DD5',
                            'color': '#fff',
                    ...
                    else if (buttonConfig.buttonText === "Cancel" || buttonConfig.buttonText === "No")
                    {
                        button.css({
                            'background': '#fff',
                            'color': '#5C5C5C',
                ...
                if (isDisabled()) {
                    ...
                    // enable custom hover if specified
                                       // Changed the dialog button hover color
                    if (isDefinedAndNotNull(buttonConfig.color)) {
                        if (buttonConfig.buttonText === "Cancel" || buttonConfig.buttonText === "No")
                        {
    ...
    var shellCloseButtonVisibilty = function(buttonVisibilty){
        var closeContainer = top.window.document.getElementById("shell-close-container");
        if(closeContainer !== null)
            closeContainer.hidden = buttonVisibilty;
    };
    ...
    var methods = {
        ...
        show: function () {
            var dialog = $(this);
            if((parent.location.pathname.indexOf("/dataintegration/summary")===-1))
                shellCloseButtonVisibilty(false);
            
            if(dialog.parent()[0].id !== "new-processor-property-container"){
                dialog.find('#normal-dialog-close-button').remove();
                dialog.find('.dialog-header').append('<button id="normal-dialog-close-button" class="close-normal pointer" '+
                        'title="Close" style="float: right;margin: 15px; font-size: 16px;"><div class="fa fa-times"></div></button>');
                dialog.find("#normal-dialog-close-button").on("click", function(){
                    var parentDialogId = dialog.find("#normal-dialog-close-button").parent().parent().attr("id");
                 if(parentDialogId==="registry-variables-dialog"){
                  closeRegistryDialog();  
                }
                else
                $("#"+parentDialogId).modal("hide");
                });
            }
            ...
            if (!$('body').find("[data-nf-dialog-parent='" + dialog.attr('id') + "']").is(':visible')) {
              ...
              if(dialog.attr("id").indexOf("policy-management")!=0 ){
            ...
            if((parent.location.pathname.indexOf("/dataintegration/summary")===-1)&&($(this).attr("id") === "processor-group-details"))   
                shellCloseButtonVisibilty(true);
        ...
        hide: function () {            
            return this.each(function () {
                ...
                if(dialog.parent()[0].id !== "new-processor-property-container"){
                    dialog.find('#normal-dialog-close-button').remove();
                    dialog.find("#normal-dialog-close-button").off("click");
                }
