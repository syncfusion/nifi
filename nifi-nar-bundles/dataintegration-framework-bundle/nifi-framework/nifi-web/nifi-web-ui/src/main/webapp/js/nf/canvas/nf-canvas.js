...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['jquery',
            ...
            function ($, d3, nfCommon, nfGraph, nfShell, nfNgBridge, nfClusterSummary, nfErrorHandler, nfStorage, nfCanvasUtils, nfBirdseye, nfContextMenu, nfActions, nfProcessGroup) {
                return (nf.Canvas = factory($, d3, nfCommon, nfGraph, nfShell, nfNgBridge, nfClusterSummary, nfErrorHandler, nfStorage, nfCanvasUtils, nfBirdseye, nfContextMenu, nfActions, nfProcessGroup,nfDialog));
            });
...
}(this, function ($, d3, nfCommon, nfGraph, nfShell, nfNgBridge, nfClusterSummary, nfErrorHandler, nfStorage, nfCanvasUtils, nfBirdseye, nfContextMenu, nfActions, nfProcessGroup,nfDialog) {
    'use strict';
    ...
    var config = {
        urls: {
            ...
            controllerBulletins: '../dataintegration-api/flow/controller/bulletins',
            kerberos: '../dataintegration-api/access/kerberos',
            oidc: '../dataintegration-api/access/oidc/exchange',
            revision: '../dataintegration-api/flow/revision',
            banners: '../dataintegration-api/flow/banners'
        }
    };
    ...
    var updateComponentScrollbar = function(){
        var documentHeight = $(document.body).outerHeight();
        var headerElementsHeight = $(".dataintegration-container").height() + $("#flow-status").outerHeight();
        var ComponentElementHeight = documentHeight - headerElementsHeight;
        $('.component-toolbar').css({
            'height': ComponentElementHeight + 'px'
        });
        var descriptionHeight = 0;
        if($(".description-section").css("display") === "block")
            descriptionHeight = $(".description-section").outerHeight();
            
        var componentContainerHeight = ComponentElementHeight - ($("#component-tab-buttons").outerHeight()+$(".description-title-section").outerHeight()+descriptionHeight);
        $('#component-permission').css({
            'height': componentContainerHeight + 'px'
        });
        
        var mainComponentHeight = componentContainerHeight - $(".searchbox-container").outerHeight();
        $('.main-components-section').css({
            'height': (mainComponentHeight-1.5) + 'px'
        });
        $('.main-server-section').css({
            'height': mainComponentHeight + 'px'
        });
        
        var templateComponentHeight = mainComponentHeight - $(".template-operations").outerHeight() - 1.5;
        $('.template-section').css({
            'height': templateComponentHeight + 'px'
        });
        
        var serverComponentHeight = mainComponentHeight -  $(".add-server-container").outerHeight();
        $('.server-section').css({
            'height': (serverComponentHeight-2) + 'px'
        });
    };
...
// define the gradient for the processor stats background
        ...
        processGroupStatsBackground.append('stop')
            ...
            $(window).on('resize', function (e) {
                if (e.target === window) {
                    ...
                    var dialogs = $('.dialog');
                    for (var i = 0, dialoglength = dialogs.length; i < dialoglength; i++) {
                        if ($(dialogs[i]).is(':visible')){
                            if($(dialogs[i].id!==null)){
                            if(dialogs[i].id.indexOf("policy-management")>-1){
                                var shellWidth=$("#shell-dialog").width();
                                var shellHeight=$("#shell-dialog").height();
                                $("#policy-management").css("width",shellWidth); 
                                $("#policy-management").css("height",shellHeight);    
                            }
                        }
                        else{
                    ...
                    for (var j = 0, dialoglength = gridElements.length; j < dialoglength; j++) {
                    ...
                    var tabsContents = [];
                    for (var k = 0, tablength = tabsContainers.length; k < tablength; k++) {
                ...
                if ($('.dialog').is(':visible') || $('#search-field').is(':focus')||$('#SearchText').is(':focus') ||$(".editable-process-group").is(":focus")) {
                    return;
                }
                ...
                if (evt.keyCode === 8 || evt.keyCode === 46) {
                    ...
                    if(window.location.protocol === "http:"){
                        if(localStorage.getItem(window.location.hostname+"-status") === "true")
                            ...
                            headerText: "Error",
                            dialogContent: "User don't have administrator privileges."
                            });
                        }
                    } else{
                        nfActions['delete'](selection);
            ...
            $.ajax({
                ...
                if (nfCommon.isDefinedAndNotNull(response.banners)) {
                    if (nfCommon.isDefinedAndNotNull(response.banners.headerText) && response.banners.headerText !== '') {
                        ...
                        var bannerHeader = $('#headertitle').text();
                        var headerText=bannerHeader+' ('+ response.banners.headerText+')';
                        $("#headertitle").prop('title', headerText);
                        if(headerText.length>58){
                            headerText = headerText.substr(0,54) + '...)';
                        }
                        $('#headertitle').text(headerText).show();
                    }
                }
                    ...
                    loadCurrentUser().done(function (currentUser) {
                        ...
                        if (currentUser.anonymous === false) {
                            // render the users name
                                ...
                                $('#current-user').text(currentUser.identity).attr("title", currentUser.identity).show();
        ...
        canWrite: function () {
            if (permissions === null){
                if (location.protocol === "https:") {
                    $("#component-container>div").find(".selected-component").removeClass("selected-component");
                    $("#component-container>div").addClass("disable-element");
                    $("#SearchContainer,#SearchText,.description-title-section").addClass("permission-changes");
                }
                return false;
            } else {
                if(permissions.canWrite === true){
                    if(location.protocol === "https:"){
                        $("#component-container>div").removeClass("disable-element");
                        $("#SearchContainer,#SearchText,.description-title-section,.action-button-permission").removeClass("permission-changes");
                    }
                    return true;
                } else{
                    if (location.protocol === "https:") {
        ...
        updateComponentToolbar : function(){
            updateComponentScrollbar();
        },
