...
(function (root, factory) {
    ...
    var config = {
        ...
        urls: {
            cluster: '../dataintegration-api/controller/cluster',
            nodes: '../dataintegration-api/controller/cluster/nodes',
        ...
        var comparer = function (a, b) {
            ...
            if (sortDetails.columnId === 'heartbeat' || sortDetails.columnId === 'uptime') {
                ...
                if (mod < 2) {
                    ...
                    aCount = nfCommon.parseCount(aSplit[0]);
                    bCount = nfCommon.parseCount(bSplit[0]);
                    return aCount - bCount;
                } else {
                    $('#cluster-nodes-table span.queued-size-title').addClass('sorted');
                    aSize = nfCommon.parseSize(aSplit[1]);
                    bSize = nfCommon.parseSize(bSplit[1]);
                    return aSize - bSize;
                }
            } else if (sortDetails.columnId === 'maxHeap' || sortDetails.columnId === 'totalHeap' || sortDetails.columnId === 'usedHeap'|| sortDetails.columnId === 'totalNonHeap' || sortDetails.columnId === 'usedNonHeap'|| sortDetails.columnId === 'ffRepoTotal' || sortDetails.columnId === 'ffRepoUsed'|| sortDetails.columnId === 'ffRepoFree' || sortDetails.columnId === 'contentRepoTotal'|| sortDetails.columnId === 'contentRepoUsed' || sortDetails.columnId === 'contentRepoFree') {
                aSize = nfCommon.parseSize(a[sortDetails.columnId]);
                bSize = nfCommon.parseSize(b[sortDetails.columnId]);
                return aSize - bSize;
            } else if (sortDetails.columnId === 'totalThreads' || sortDetails.columnId === 'daemonThreads'
                || sortDetails.columnId === 'processors') {
                aCount = nfCommon.parseCount(a[sortDetails.columnId]);
                bCount = nfCommon.parseCount(b[sortDetails.columnId]);
                return aCount - bCount;
