...
(function (root, factory) {
    ...
    return function (serviceProvider, navigateCtrl, operateCtrl) {
        ...
        var getStyle = function(selection){
            ...
            if(checkImageExists('images/' + nf.Common.substringAfterLast(data.component.type, '.') + '.svg'))
                return "url(images/"+ nf.Common.substringAfterLast(data.component.type, '.')+".svg)";
            else
                return "url(images/CustomProcessor.svg)";   
        };
        ...
        GraphControlsCtrl.prototype = {
            ...
            getContextIcon: function () {
                ...
                var selectedPolicy=$('#selected-policy-item-type').text();
                if (selection.empty()||selectedPolicy ==='controller-services'|| selectedPolicy ==='reporting-tasks') {
                    if (nfCanvasUtils.getParentGroupId() === null ||selectedPolicy ==='controller-services'|| selectedPolicy ==='reporting-tasks') {
                        return 'icon-drop';
                    } else {
                        return 'icon-group';
                    }
            ...
            getContextName: function () {
                var selectedPolicy=$('#selected-policy-item-type').text();
                 if(selectedPolicy === 'controller-services'|| selectedPolicy === "reporting-tasks"){
                         return $("#selected-policy-item-name").text();
                  }
                else{
                var selection = nfCanvasUtils.getSelection();
        ...
        /**
         * Gets the style to added for selected processor.
         */
        getIcon: function () {
            var selection =nfCanvasUtils.getSelection();
            ...
            if (!selection.empty()) {
                 if (selection.size() === 1) {
                    if (nfCanvasUtils.isProcessor(selection)) {
                      return {
                          "background-image": getStyle(selection),
                            "width":"18px",
                            "height":"18px",
                            "float":"left",
                        };
