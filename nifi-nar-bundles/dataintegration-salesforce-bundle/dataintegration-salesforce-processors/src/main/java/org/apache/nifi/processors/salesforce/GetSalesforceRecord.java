...
@Tags({"salesforce","sobject","fieldsvalue"})
@CapabilityDescription("Retrieves the field values from a salesforce record. Specify the fields you want to retrieve in the fields parameter.")
public class GetSalesforceRecord extends AbstractSalesForceProcessor {
    ...
    public static final PropertyDescriptor SOBJECT_ID = new PropertyDescriptor
            ...
            .description("Enter Salesforce object id to get corresponding record.")
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .expressionLanguageSupported(true)
            .required(true)
            .build();
    ...
    public static final PropertyDescriptor SOBJECT_FIELD = new PropertyDescriptor
            .Builder()
            .name("Salesforce Object Field")
            .description("Enter multiple salesforce object fields with comma (,) as delimiter.E.g. field1,field2")
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .expressionLanguageSupported(true)
            .required(false)
            .build();
    ...
    protected void init(final ProcessorInitializationContext context) {
        ...
        propertyDescriptor.add(SOBJECT_FIELD);
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            String sobjectFields = context.getProperty(SOBJECT_FIELD).evaluateAttributeExpressions().getValue();
            if(sobjectFields == null || sobjectFields.isEmpty())
                sobjectFields = trimToEmpty(context.getProperty(SOBJECT_FIELD).evaluateAttributeExpressions(flowFile).getValue());
            
            String endpoint = SALESFORCE_OP + "/" + sobjectName + "/" + sobjectId + "?fields=" + sobjectFields;
