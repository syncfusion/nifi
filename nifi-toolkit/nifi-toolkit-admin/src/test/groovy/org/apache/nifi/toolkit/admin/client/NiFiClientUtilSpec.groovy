...
class NiFiClientUtilSpec extends Specification{
    ...
    def "build unsecure url successfully"(){
        ...
        when:
        def url = NiFiClientUtil.getUrl(niFiProperties,"/dataintegration-api/controller/cluster/nodes/1")

        then:

        3 * niFiProperties.getProperty(_)
        url == "http://localhost:8080/dataintegration-api/controller/cluster/nodes/1"
    }
